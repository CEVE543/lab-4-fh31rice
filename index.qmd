...
Ans: Houston faces extrem rainfall in 2017 in Harvey, where I saw from NOAA data that the max one day rainfall was 16.07 inch in one day...For our GEV we are look max daily annual rainfall in a year...

.Now say if I imagine X10(10 year return period)=11 inch(just assume an event less than harvey rainfall), X50(say harvey is 50 year event)=17 inch and shape=0.05(I assume it to be small considering that the tail will certainly remain bounded like gumbel or frechet, if shape is very high then the return level will be unbounded which shouldn't be a case), then from equation of inverse CDF, there will be unknown paramter meu and sigma(bz we know shape parameter and XT as we assumed)...so for two equations X10 and X50 in inverse CDF, two unknown can be solved, the value can come meu as 3.58(location parameter) and sigma(Scale parameter) can be 3.1 if we solve those equations....

Now, we are not sure that if 17 inch is 50 year or 100 year event, in that case if we consider 17 inch as X100 and 11 inch as X10, the value of meu and sigma will change to say 5.8 and 2.1...so consider location parameter as Normal(5,2) means the bound is  5-2sigma=5-(2x2)1 and 5+2sigma=5+(2x2), that can cover rainfall of 3.58 and 5.8 in this range...

for scale parameter, the dist is log(sigma)=Normal(m,n). So say, log_σ ~ Normal(0.5, 0.4); that means that mean is 0.5 and sigma is 0.4 for shape parameter, so (mean±2sigma) is in 0.5 ± 0.8 = [-0.3,1.3], now exp(-0.3)=0.74 and exp(1.3)=3.67 which cover our previous value of 3.1 and 2.1..

And for shape parameter let's consider we dont have very heavy tail bz onbly harvey got a very high rainfall, so we dont need to consider unbounded tail that much, that's why I consider shape parameter as (0.05,0.10),

If these parameters are considered, I think the graph looks much better
...
Ans: Here, when we were considering very bad value for location, scale and shape parameters, to generate fake rainfall and returnh period, the graph was going very unrealistic, say like 10 year event was going for 75 inch rainfall, 250 year event was going for 10 inch rainfall, which shouldn't be true, because with increase in return period the return level will increase.

But, when I saw max one day rainfall data of aHarvey in last 30 years, I assume that this one day rainfall canb be a 50 or 100 year return period event, and then any rainfall less than that say 11 inch can be a 10 year event, Then I tried to use the concept of inverse CDF to calculate the location and scale parameter for two equations two unknown concept (X10 and X50); (X10 and X100) considering shape parameter as 0.05(or 0 also can be assumed for gumbel, which will just make location and scale parameter unknow for inverse CDF)....Then calculating two set of location and scale parameter for (X10,X50) and (X10,X100), I tried to guess the mean and std dev which is described in Ques 1....So domain knowledge(NOAA report) helps to update my belief(priors into posteriors).
...
Ans: We should use different priors for different stations(Say one is located in coastal, one in inland) because climate varies in place to place. But if the stations are located in the same region, we can use the same prior.
...
Ans: R value 1 means, model is converging. For MLE,, we have just only one estimation ,there is no uncertainty quantification associated with it. But for Bayesian, we get bands of uncertainty for prior and posterior, because those comes from a distribution.
...
Ans: Iterative workflow is challenging, because it is always difficult to have some prior distributiona ssumption for location, scale and shape parameter because data can be missing from stations.
...
#| output: false
using Pkg
lab_dir = "D:\\FALL 2025\\CEVE 543\\labs\\lab-4-fh31rice" # <1>
...
```{julia}
my_stnid = 112  # REQUIRED: replace with your chosen station ID
...
:::
Ans: So, these two graphs plot about retur level vs return [eriod priors, here wehn we gave prior to retunr period, the uncertainties bound in right picture is mpre realistic, the 250 year RP uncertainty lies in between 8 to 18, because return period and retunr level is more  easy to assume rather thanb assuming location, scale, and shape parameters.]
## Inference 
...
```{julia}
comparison_stnid = 121 # MODIFY THIS AND CHOOSE YOUR COMPARISON
...
:::
Ans: For Austin the shape parameter(solid line is more unbounded, heavier tails, higher positive ξ) and facing more return level value for 250 years, but for Bay city, seems like the solid line is unbounded but with slighlt lower shape parameter value.

Regarding the uncertainty(scale parameter), both the stations range seems to be very much similar pattern in terms of width.

Regarding location paramter, austin is facing more central tendency.

Austin, here is facing more return level maybe because of observing more tropical storm in city area.


We should use different priors for different stations because climate varies in place to place.

Yes, there is a overlap, say for 25 year return period both stations face return level of around 8 inch. But with increas in return period , the overlap reduces

Spatial variability is important, because say like some stations might face heavy thunderstorm and that can shift the tail behaviour more, in those places, risk is also higher.
